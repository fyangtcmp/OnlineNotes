## 自动数组 (automatic array)

我们知道，Fortran 作为一门编译型语言，其中数组的尺寸在赋值、运算前都是需要预先确定的。

> 注：Fortran  2008 规范中引入了和解释性语言类似的、在赋值时根据等号右侧数组的大小，自动改变等号左侧数组大小的行为，但如果是无意中使用了这一行为并导致了 bug 的话，后续 debug 会非常困难，所以个人推荐使用编译器选项 `-nostandard-realloc-lhs` 禁用这一行为，更多解释可以参照 ifort 编译器手册对于这一选项的解释。

而确定尺寸的“时机”又可以分为两类，一类是静态尺寸的数组，在编译时确定，例如
```fortran
...
real(dp) :: mat(100)
```
另一类是动态尺寸的数组，在运行时确定，标准做法是通过 allocate 的方式，例如
```fortran
real(dp), allocatable :: mat(:)
A = 100
allocate (mat(A))
```
不过，Fortran 2018 规范（最早引入的时间未知）还存在着一种像静态尺寸数组那样声明动态尺寸数组的语法，称为自动数组 (automatic array)。具体而言，如果一个数组是定义在 subroutine 或者 function 里的，那么它可以接受来自 program 或者 module 里的**变量**，或者是一个输入的**变量**，来直接指定维度，例如
```fortran
module A_module
integer :: A
end module
